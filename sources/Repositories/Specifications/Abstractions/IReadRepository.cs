...
public partial interface IReadRepository<TEntity>
{
	...
	ValueTask<Result<TResult>> FirstAsync<TResult>(
		[NotNullWhen(false)] ISpecification<TEntity, TResult> specification,
		CancellationToken cancellationToken = default);
	...
	ValueTask<Result<TEntity>> SingleAsync(
		[NotNullWhen(false)] ISingleResultSpecification<TEntity> specification,
		CancellationToken cancellationToken = default);
	...
	ValueTask<Result<TResult>> SingleAsync<TResult>(
		[NotNullWhen(false)] ISingleResultSpecification<TEntity, TResult> specification,
		CancellationToken cancellationToken = default);
	...
	/// </returns>
	Result<IAsyncEnumerable<TEntity>> StreamAsync([NotNullWhen(false)] ISpecification<TEntity> specification);
	...
	/// Returns an <see cref="IAsyncEnumerable{T}"/> which can be enumerated asynchronously.
	/// </returns>
	Result<IAsyncEnumerable<TResult>> StreamAsync<TResult>([NotNullWhen(false)] ISpecification<TEntity, TResult> specification);
