...
public partial interface ISpecification<T, TResult> : ISpecification<T>
{
	...
	/// </summary>
	Option<Expression<Func<T, TResult>>> Selector { get; }

	/// <summary>
	/// The SelectMany transform function to apply to the <typeparamref name="T"/> element.
	/// </summary>
	Option<Expression<Func<T, IEnumerable<TResult>>>> SelectorMany { get; }

	/// <summary>
	/// The transform function to apply to the result of the query encapsulated by the <see cref="ISpecification{T, TResult}"/>.
	/// </summary>
	new Option<Func<IEnumerable<TResult>, IEnumerable<TResult>>> PostProcessingAction { get; }

	new Result<IEnumerable<TResult>> Evaluate(IQueryable<T> entities);
}
...
public interface ISpecification<T>
{
	...
	/// </summary>
	Option<Func<IEnumerable<T>, IEnumerable<T>>> PostProcessingAction { get; }
	...
	/// <returns></returns>
	Result<IEnumerable<T>> Evaluate(IQueryable<T> entities);
